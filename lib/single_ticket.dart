...
class _SingleTicketDetailsScreenState
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data.isNotEmpty) {
          ...
          final player = PlaySound(soundPath:'sounds/scan-error.mp3');
          player.play();
          setState(() => isLoading = false);
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('No ticket details found.'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                response.statusCode == 401 ? 'Not authorised for this event' : 'Unable to load ticket (Error ${response.statusCode}).'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    if (ticket == null) {
      return Scaffold(
        appBar: AppBar(title: const Text('Single Ticket')),
        body: const Center(child: Text('Ticket details unavailable.')),
      );
    }
  ...
  Future<void> punchSingleTicket(String uuid) async {
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        final player = PlaySound(soundPath: 'sounds/admitted.mp3');
        player.play();
